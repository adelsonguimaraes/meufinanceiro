...
Class meta_dao {
	...
	public function __construct($con) {
		$this->con = $con;
		$this->superdao = new super_dao('meta');
	}
	...
	//buscarPorId
	function buscarPorId (meta $obj) {
		$this->sql = sprintf("SELECT * FROM meta WHERE id = %d",
			mysqli_real_escape_string($this->con, $obj->getId()));
	...
	function listar ($idusuario) {
		...
		if(!$result) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'meta' , 'Listar' ) );
		}else{
			while($row = mysqli_fetch_object($result)) {
	...
	function listarPaginado($idusuario, $start, $limit) {
		...
		if ( !$result ) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'meta' , 'ListarPaginado' ) );
		}else{
	...
	function deletar (meta $obj) {
		...
		$this->sql = sprintf("DELETE FROM meta WHERE id = %d",
			mysqli_real_escape_string($this->con, $obj->getId()));
		$result = mysqli_query($this->con, $this->sql);
	...
	//quantidade total
	function qtdTotal() {
		$this->sql = "SELECT count(*) as quantidade FROM meta";
		$result = mysqli_query ( $this->con, $this->sql );
		if (! $result) {
