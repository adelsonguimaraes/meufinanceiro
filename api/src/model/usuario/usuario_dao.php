...
Class usuario_dao {
	...
	public function __construct($con) {
		$this->con = $con;
		$this->superdao = new super_dao('usuario');
	}

	//cadastrar
	function cadastrar (usuario $obj) {
		$this->sql = sprintf("INSERT INTO usuario(nome, email, senha, seguranca)
		VALUES('%s', '%s', '%s', %f)",
			...
			mysqli_real_escape_string($this->con, $obj->getSeguranca()));
	...
	function atualizar (usuario $obj) {
		$this->sql = sprintf("UPDATE usuario SET nome = '%s', email = '%s', email = %f, dataedicao = '%s' WHERE id = %d ",
			mysqli_real_escape_string( $this->con, $obj->getNome() ),
			mysqli_real_escape_string( $this->con, $obj->getEmail() ),
			mysqli_real_escape_string($this->con, $obj->getSeguranca()),
	...
	function atualizarMeusDados (usuario $obj) {
		...
		$this->sql .= ", seguranca = {$obj->getSeguranca()}";
	...
	function buscarInfoPageConsultor ($usuario, $identificador) {
		...
		if(!$result) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), "usuario_dao", 'buscarInfoPageConsultor' ) );
		}else{
			while($row = mysqli_fetch_object($result)) {
				$this->obj = $row;
			}
	...
	function acessar ( $email, $senha ) {
		...
		if( !$result ) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), get_class( $obj ), 'Acessar' ) );
		}else{
	...
	function setMenuConsultor ($idusuario) {
		...
		if(!mysqli_query($this->con, $this->sql)) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), "usuario_dao", 'setMenuConsultor' ) );
		}else{
			$id = mysqli_insert_id( $this->con );
