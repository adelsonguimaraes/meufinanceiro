...
Class TipotaxaDAO {
	...
	public function __construct($con) {
		$this->con = $con;
		$this->superdao = new super_dao('tipotaxa');
	}

	//cadastrar
	function cadastrar (tipotaxa $obj) {
		$this->sql = sprintf("INSERT INTO tipotaxa(descricao)
		VALUES('%s')",
	...
	//atualizar
	function atualizar (Tipotaxa $obj) {
		$this->sql = sprintf("UPDATE tipotaxa SET descricao = '%s', dataedicao = '%s' WHERE id = %d ",
			mysqli_real_escape_string($this->con, $obj->getDescricao()),
	...
	//buscarPorId
	function buscarPorId (Tipotaxa $obj) {
		$this->sql = sprintf("SELECT * FROM tipotaxa WHERE id = %d",
			mysqli_real_escape_string($this->con, $obj->getId()));
	...
	//listar
	function listar () {
		$this->sql = "SELECT * FROM tipotaxa";
		$result = mysqli_query($this->con, $this->sql);

		$this->superdao->resetResponse();

		if(!$result) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'Tipotaxa' , 'Listar' ) );
		}else{
			while($row = mysqli_fetch_object($result)) {
	...
	function listarPaginado($start, $limit) {
		$this->sql = "SELECT * FROM tipotaxa limit " . $start . ", " . $limit;
		$result = mysqli_query ( $this->con, $this->sql );

		$this->superdao->resetResponse();

		if ( !$result ) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'Tipotaxa' , 'ListarPaginado' ) );
		}else{
	...
	function deletar (Tipotaxa $obj) {
		...
		$this->sql = sprintf("DELETE FROM tipotaxa WHERE id = %d",
			mysqli_real_escape_string($this->con, $obj->getId()));
		$result = mysqli_query($this->con, $this->sql);
	...
	//quantidade total
	function qtdTotal() {
		$this->sql = "SELECT count(*) as quantidade FROM tipotaxa";
		$result = mysqli_query ( $this->con, $this->sql );
		if (! $result) {
