...
function data ($timeout) {
    return function (input) {
        if(!input || typeof(input) == 'object') return input;
        input = input.replace('T', ' '); // SE 0000-00-00T00:00:00 -> 0000-00-00 00:00:00
        if (input.toString().indexOf(':')===-1) input = input + ' 00:00:00'; // 0000-00-00 -> 0000-00-00 00:00:00
...
function dia_mes () {
    return function (input) {
        ...
        const meses = [
            {name: "JANEIRO", abreviacao: 'JAN'}, {name: "FEVEREIRO", abreviacao: 'FEV'}, {name: "MARÇO", abreviacao: 'MAR'}, {name: "ABRIL", abreviacao: 'ABR'},
            {name: "MAIO", abreviacao: 'MAI'}, {name: "JUNHO", abreviacao: 'JUN'}, {name: "JULHO", abreviacao: 'JUL'}, {name: "AGOST", abreviacao: 'AGO'},
            {name: "SETEMBRO", abreviacao: 'SET'}, {name: "OUTUBRO", abreviacao: 'OUT'}, {name: "NOVEMBRO", abreviacao: 'NOV'}, {name: "DEZEMBRO", abreviacao: 'DEZ'}
        ];
        return `${split[2]} ${meses[Number(split[1])-1].abreviacao}`;
    }
}
